...
Narcissus.resolver = (function() {
    ...
    function StaticEnv(type, parent) {
        this.type = type || GLOBAL_FRAME;
        this.parent = parent || null;
        this.frame = new StringMap();
        // cache the nearest enclosing script frame
        this.scriptFrame = this.isScriptFrame() ? this : parent.scriptFrame;
        // all free variables are stored in the global environment frame
        this.freeVars = parent ? parent.freeVars : new StringMap();
    }
    ...
    function resolveCatch(node, env) {
        let env2 = new StaticEnv(CATCH_FRAME, env);
    ...
    function resolveModule(node, env) {
        if (node.body) {
            let env2 = new StaticEnv(MODULE_FRAME, env);
    ...
    function resolveFunction(node, env) {
        let env2 = new StaticEnv(FUNCTION_FRAME, env);
