...
ExecutionContext.prototype = {
    ...
    evalBinOp: function (v1, v2, op) {
        return eval('v1 ' + op + ' v2');
    },

    // Only handles +, -, ! and ~ opeartions
    evalUnaryOp: function (v, op) {
      return eval(op + "v");
    },
...
function execute(n, x) {
    ...
    switch (n.type) {
      ...
      case IF:
      //TODO: add faceted functionality to this
        if (x.getValue(execute(n.condition, x)))
      ...
      case SWITCH:
        s = x.getValue(execute(n.discriminant, x));
        a = n.cases;
        ...
        for (i = 0, j = a.length; ; i++) {
            ...
            if (t.type === CASE) {
                u = x.getValue(execute(t.caseLabel, x));
            } else {
      ...
      case FOR:
        n.setup && x.getValue(execute(n.setup, x));
        // FALL THROUGH
      case WHILE:
        while (!n.condition || x.getValue(execute(n.condition, x))) {
            try {
                execute(n.body, x);
            } catch (e if e === BREAK_SIGNAL && x.target === n) {
                break;
            ...
            n.update && x.getValue(execute(n.update, x));
        }
        break;
      ...
      case DO:
        ...
        } while (x.getValue(execute(n.condition, x)));
        break;
      ...
      case TRY:
        try {
            ...
            for (i = 0; ; i++) {
                ...
                try {
                    if (t.guard && !x.getValue(execute(t.guard, x)))
                        continue;
                    execute(t.block, x);
                    break;
                } finally {
      ...
      case THROW:
        throw x.getValue(execute(n.exception, x));

      case RETURN:
        // Check for returns with no return value
        x.result = n.value ? x.getValue(execute(n.value, x)) : undefined;
        throw RETURN_SIGNAL;

      case WITH:
        r = execute(n.object, x);
        t = toObject(x.getValue(r), r, n.object);
        x.scope = {object: t, parent: x.scope};
        try {
            execute(n.body, x);
        } finally {
      ...
      case SEMICOLON:
        if (n.expression)
            x.result = x.getValue(execute(n.expression, x));
        break;
      ...
      case HOOK:
        c = n.children;
        v = x.getValue(execute(c[0], x)) ? x.getValue(execute(c[1], x))
            : x.getValue(execute(c[2], x));
        break;
      ...
      case MOD:
        c = n.children;
        v1 = x.getValue(execute(c[0], x));
        v2 = x.getValue(execute(c[1], x));
        v  = x.evalBinOp(v1, v2, ops[n.type]);
        break;

    case IN:
      c = n.children;
      v = x.getValue(execute(c[0], x)) in x.getValue(execute(c[1], x));
      break;

    case INSTANCEOF:
      c = n.children;
      t = x.getValue(execute(c[0], x));
      u = x.getValue(execute(c[1], x));
      ...
      case VOID:
        x.getValue(execute(n.children[0], x));
        break;
      ...
      case UNARY_PLUS:
      case UNARY_MINUS:
        v = x.getValue(execute(n.children[0], x));
        v = x.evalUnaryOp(v, ops[n.type])
        break;

      case INCREMENT:
      case DECREMENT:
        t = execute(n.children[0], x);
        u = Number(x.getValue(t));
        if (n.postfix)
            v = u;
      ...
      case OBJECT_INIT:
        ...
        for (i = 0, j = c.length; i < j; i++) {
            ...
            if (t.type === PROPERTY_INIT) {
                let c2 = t.children;
                v[c2[0].value] = x.getValue(execute(c2[1], x));
            } else {
                f = newFunction(t, x);
...
exports.repl = repl;
exports.test = test;
// ExecutionContext properties can be altered to get alternate behaviour.
